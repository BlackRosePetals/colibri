 ...
 *  gs/us/ds[e] = pointers to expert e's scales: per-row [O] for fmt 1/2, per-group
 *                [O,ceil(K/gs4)] for fmt=4 (K = the matmul's input dim)
 *  fmt         = quant format (shared across experts): {1,2,4,6}; anything else
 *                returns 0 (CPU fallback)
 *  gs4         = fmt=4 group size along the input dim (shared across experts and the
 *                three matrices). Must be even and >=2; pass <=0 to force the CPU
 *                fallback (the caller poisons it on mixed per-expert group sizes).
 *                Ignored for other fmts.
 ...
 */
int coli_metal_moe_block(int nb, int D, int Iinter, int fmt, int gs4,
...
ColiMetalMoeHandle* coli_metal_moe_block_begin(int nb, int D, int Iinter, int fmt, int gs4,
                         const void *const *g, const void *const *u, const void *const *d,
                         const float *const *gs, const float *const *us, const float *const *ds,
                         const float *xg, const int *xoff, const int *nr,
                         const int *rows, const float *rw);
